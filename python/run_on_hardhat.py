...
class ToolRunData:
    ...
    def to_summary_json(self):
        """
        Provide this data as a JSON dict
        """
        result = {
            "name": self.tool_name,
            "num_sources": len(self.sources),
    ...
    def print_summary(self, short=False):
        ...
        n_fail = len(failures)
        total = n_succ + n_fail
        r_succ = n_succ / total * 100
        r_fail = n_fail / total * 100
        if not short:
            if successes:
            ...
            if failures:
                print(f"Failures for {self.tool_name}:")
                for source in failures:
                    print(
                        f"    [\033[31;1m - \033[0m] {source} ({self.num_errors[source]} errors)"
                    )
        print(f"    SUCCESSES: {n_succ:3} / {total:3} ({r_succ:6.2f}%)")
        print(f"    FAILURES:  {n_fail:3} / {total:3} ({r_fail:6.2f}%)")
...
def main():
    ...
    # Print an overview
    for tool in (solc_run_data, solang_parser_run_data, gambit_run_data):
        if tool is None:
            continue
        print(f"\033[33;1mSummary of {tool.tool_name}\033[0m")
